...
- `sources/`: 웹/SNS 클리핑 Markdown, PDF 원본, PDF 변환 Markdown 같은 원자료
- `llm-wiki/`: LLM이 정리한 위키 노트와 산출물
- `graphify-out/`: Graphify 리포트, JSON, HTML, 캐시

위 경로는 `.gitignore`로 내용 파일을 제외합니다. 다만 clone 사용자가 폴더 구조와 Web Clipper 설정을 볼 수 있도록 `.gitkeep`과 `sources/clipper-*.json`은 추적합니다.
...
| `sources/` | 웹/SNS 클리핑 자료와 PDF 기반 원자료를 보관하는 입력 폴더입니다. `clipper-*.json`은 Obsidian Web Clipper 설정/분류 파일, `web/`과 `sns/`는 클리핑 Markdown, `papers/pdf/`는 사용자가 직접 다운로드한 PDF 원본, `papers/`는 Codex가 PDF를 Markdown으로 변환해 저장하는 위치입니다. | 실제 자료는 제외하고 `.gitkeep`과 `clipper-*.json`만 추적합니다. |
...
1. Obsidian Web Clipper를 활용하여 관심 있는 웹/SNS 자료를 `sources/sns/`, `sources/web/` 폴더에 클리핑합니다.
2. 웹에서 직접 클리핑하기 어려운 PDF는 사용자가 다운로드해 `sources/papers/pdf/` 폴더에 저장합니다.
3. PDF 원본을 정리할 때는 Codex에게 요청해 Markdown 변환본을 `sources/papers/`에 저장합니다.
4. Codex에서 LLM Wiki 스킬로 `sources/` 자료 ingest를 수행합니다.
5. `llm-wiki/wiki/`에 source, concept, entity, idea, decision 노트를 축적합니다.
6. LLM Wiki 기반으로 Graphify 지식 graph를 생성합니다.
7. 기획 아이디어를 제시합니다. 예: “AI 에이전트 기업 내 도입방안”
8. LLM Wiki, gstack `office-hours`, Superpowers `brainstorming`을 활용해 아이디어를 구체화합니다.
9. 필요한 경우 `llm-wiki/outputs/docs/`, `llm-wiki/outputs/slides/`, `llm-wiki/outputs/apps-script/`에 산출물을 작성합니다.
...
대표 차이점은 gstack이 의사결정과 검토자 역할에 가깝고, Superpowers는 작업 수행 방법론에 가깝다는 점입니다.

| 구분 | gstack | Superpowers |
| --- | --- | --- |
| 중심 역할 | 판단, 검토, 반박, 의사결정 | 계획, 구현, 디버깅, 검증 |
| 좋은 사용처 | 아이디어 압박 검증, 전략 리뷰, QA/출시 판단 | 코드 작성, 실행계획, 테스트, 문제 해결 |
| 대표 느낌 | "이 아이디어가 진짜 괜찮은가?" | "그럼 어떻게 안전하게 만들고 검증할까?" |
| 프로젝트 기본 조합 | `office-hours` | `brainstorming` |
...
```text
sources/papers/pdf/ 안의 PDF를 읽고 Markdown으로 정리해서 sources/papers/에 저장해줘.
원본 PDF는 수정하지 말고, 이후 LLM Wiki ingest에 사용할 수 있도록 제목, 출처, 핵심 요약, 주요 내용, 인용할 만한 포인트를 포함해줘.
```
...
- Web Clipper 설정 파일 `sources/clipper-*.json`
- `.gitignore`
